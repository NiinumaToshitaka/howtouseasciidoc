...
https://hub.docker.com/r/asciidoctor/docker-asciidoctor/[asciidoctor/docker-asciidoctor - Docker Image | Docker Hub] で最新のイメージのバージョンを確認する。
...
vscodeの https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers[Remote - Containers] 拡張機能を使用して、コンテナ内に環境を構築する。
この拡張機能の仕組みは https://code.visualstudio.com/docs/remote/containers[Developing inside a Container using Visual Studio Code Remote Development] を参照。

==== 拡張機能をインストール

拡張機能画面から"Remote - Containers"をインストールする。
