...
AsciiDoctorコンテナとKrokiコンテナの間で通信する必要があるので、ブリッジネットワークを作成する。
ここでは `asciidoctor` という名前で作成する。

[source, console, linenums]
----
$ docker network create asciidoctor
4f41b56b14e5be913ce37ad96979ae22bfca47735029b383cf9893542350c5f1
----
...
docker run \
  --rm \#<1>
  --name kroki \#<2>
  --publish {kroki-publish-port}:8000 \#<3>
  --network asciidoctor \#<4>
  --detach \#<5>
  yuzutech/kroki:0.17.2 #<6>
----
<1> 実行終了時にコンテナを削除する（コンテナを残しておく必要がないため）
<2> コンテナ名を指定する（必須ではないが名前を固定したほうがわかりやすい）
<3> ホスト側に公開するポート番号を割り当てる。{kroki-publish-port}番が既に使用されている場合は別のポート番号を割り当てる。たとえば `--publish 1234:8000` とすると、ホスト側からは http://localhost:1234 からアクセスできる。
<4> <<create-bridge-network>>で作成したネットワークに接続する
<5> バックグラウンドでコンテナを実行する
<6> https://hub.docker.com/r/yuzutech/kroki[Kroki公式のイメージ配布ページ] から、最新のイメージのバージョンを確認して使用する

上記コマンド実行後、ホスト側から http://localhost:{kroki-publish-port} にアクセスできることを確認する。

[CAUTION]
====
`yuzutech/kroki` イメージ単体だと、PlantUMLの図はレンダリングできるが、Mermaidなどいくつかの図はレンダリングできない。他のコンテナを立てる必要がある。

他のコンテナを立てる手順は<<run-multiple-kroki-containers-together>>を参照。
====

==== Krokiで画像を生成する

<<use-docker-image-of-asciidoctor>>で環境を構築しているものとする。

AsciiDoctorコンテナとKrokiコンテナの間で通信可能にするために、AsciiDoctorコンテナとKrokiコンテナを同じDockerブリッジネットワークに接続する必要がある。
AsciiDoctorコンテナを<<create-bridge-network>>で作成したブリッジネットワークに接続するために、  `.devcontainer.json` に以下を追加する。
...
:allow-uri-read:
----
<1> コンテナ間の通信であるため、<<run-kroki-container>>でホスト側に公開するポート番号を変更していても、ここではKrokiコンテナ側のポート番号である8000を指定する。
